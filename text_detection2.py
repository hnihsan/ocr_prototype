...
def start_ocr():
    ...
    while ok_flag:
        ...
        hImg, wImg, _ = filtered.shape
        boxes = pytesseract.image_to_boxes(filtered)
        for b in boxes.splitlines():
            b = b.split(' ')
            x, y, w, h = int(b[1]), int(b[2]), int(b[3]), int(b[4])
            cv2.rectangle(filtered, (x, hImg - y), (w, hImg - h), (50, 50, 255), 2)
            cv2.putText(filtered, b[0], (x, hImg - y + 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (50, 50, 255), 2)

        result_string = pytesseract.image_to_string(filtered)
